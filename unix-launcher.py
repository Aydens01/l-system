...
Retourne vrai seulemeunt si l'objet entré ne contient que
les characteres autorisés
"""
def checkAxiome(objet) :
    for i in objet :
        if not i in "ab+-*[]&à" :
            return(False)
    return(True)
...
Fonctions un peu plus longue car on y a rajouté 2 characteres de changement de taille
et une image pour la tortue.
"""
def ecriture(figure, taille, angle, sortie) :
    grosseur, R, G, B = 1, 1.00, 0.00, 0.00
    sortie.write("from turtle import * \npositions, angles = [], [] \n")
    sortie.write("bgcolor(0.1,0.1,0.1); speed(0); register_shape('img/cat.gif'); shape('img/cat.gif'); pensize(" + str(grosseur) + ")\n")
    ...
    for i in figure :
        ...
        if i == "a" : sortie.write("pd();fd(" + str(taille) + ");\n")
            ...
            sortie.write("setpos(positions[-1]);setheading(angles[-1]);\n")
...
entree = open('rules/'+nom_entree,"r")
sortie = open('orders/'+nom_sortie,"w")
...
os.system("python3 orders/"+str(nom_sortie))
